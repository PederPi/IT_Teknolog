///////////////////////////////////////////////////
//    Author: Jonas "Peder" Iversen, 21/10-23    //
///////////////////////////////////////////////////
...
#include <avr/sleep.h>  //Library for handling low power sleep of the arduino between logging

//Defines and declarations. 
#define ALARM_PIN 2                   //Alarm interupt from RTC(SQW)
#define DHT_IN_PIN 3                     //Data pin from DHT sensor
#define DHT_OUT_PIN 4
#define CHIPSELECT_PIN 53             //Chip select pin for the sd card reader.
#define LOGGING_INTERVAL 10          //Interval between logging data to file. (Warning do not change!)
...
const bool DEBUG = false;             //Enable or disable code debugging, if enables will send information to serial.
RTC_DS3231 rtc;                       //Create object of type: "RTC_DS3231" with name: "rtc"
DHT dht_OUT(DHT_OUT_PIN, SENSOR_TYPE);        //Create object of type: "DHT" with name: "DHT22"
DHT dht_IN(DHT_IN_PIN, SENSOR_TYPE);        //Create object of type: "DHT" with name: "DHT22"
...
//Main:    
void loop() {
  sleep();                          //Put the Arduino to sleep.
  delay(10000);
...
void rtcInit(){
  ...
  rtc.disableAlarm(1);                              //Disable alarm 1 on the RTC
  rtc.disableAlarm(2);                              //Disable alarm 2 on the RTC
  rtc.clearAlarm(1);                                //Clear alarm 1 on the RTC
  rtc.clearAlarm(2);                                //Clear alarm 2 on the RTC
  rtc.writeSqwPinMode(DS3231_OFF);                  //Change the squarewave pin on the RTC from 1Hz squarewave to interupt
...
void sleep(){
  ...
  while(run){
    ...
    rtc.setAlarm1(DateTime(0, 0, 0, 0, 0, 50), DS3231_A1_Second); //Sets and alarm to the user defined logging interval minus the delay.
    sleep_enable();                       //Enabling sleep mode.
    set_sleep_mode(SLEEP_MODE_PWR_DOWN);  //Set sleep mode to power down.
    noInterrupts();                       //Disable interrupts.
    attachInterrupt(digitalPinToInterrupt(ALARM_PIN), alarm_ISR, LOW);  //Set interupt to the alarm pin from the RTC when LOW.
    interrupts();                         //Enable interupts.
    sleep_cpu();                          //Enter sleep mode  
    rtc.disableAlarm(1);                  //Disable and clear the alarm to reset it.
    rtc.clearAlarm(1);
    if((rtc.now().minute() + 1) % LOGGING_INTERVAL == 0){
      run = 0;
    }
  }
}

//Function:    ISR function for the RTC alarm.
void alarm_ISR() {  
